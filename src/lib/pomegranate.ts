...
async function withWallTimeout<T>(promise: Promise<T>, ms: number, onTimeout: () => T): Promise<T> {
  let timer = 0
  try {
...
export function authenticateWithGooglePopup(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    if (!popup) {
      settle(() => reject(new Error('Popup blocked — allow popups for this site and try again')))
      return
    }
